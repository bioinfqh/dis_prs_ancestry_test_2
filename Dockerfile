...
#&& chmod 777 /ancestry_mirror-master/src/ancestry 
...
RUN apt-get update && apt-get install -y python3.7 && apt-get install -y python3-pip && pip3 install -r requirements.txt && apt autoremove && apt autoclean && apt-get clean packages && apt-get install -y curl && apt-get install -y libc6-dev
