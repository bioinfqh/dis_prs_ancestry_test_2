...
void printopts(){
	...
	cout << "-e [float] ancestry proportion below which population is dropped (5e-3)\n";
	...
	cout << "-printVCF print out a VCF with individual-specific allele frequencies\n";
	cout << "-merge merge ancestry from similar clusters\n";
	cout << "\n";
}
...
int main(int argc, char *argv[]){
    ...
    string bla = "bla";
    string bla2 = "bla2";   
    printf("%s\n",bla.c_str());    // read the reference panel
    FreqData cd(&p);
    p.print();
    if (!p.bootstrap){
    		...
    		if (p.merge){
    			bool merged = f23.mergeclusters();
    			int mergecount = 1;
    			while (merged and mergecount < 10){
    				merged = f23.mergeclusters();
    			}
    		}
    		//print a VCF with the individual allele frequencies
    		if (p.printVCF) {
    			map<string, float> weights = f23.return_Q();
    			Data23 f23(&p, &cd, r);
    			f23.printVCF(p.outstem+".vcf.gz", weights);
    		}
